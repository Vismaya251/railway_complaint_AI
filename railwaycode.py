...
def categorize_complaint(complaint_text):
    # List of keywords indicating non-railway context
    non_railway_keywords = ["university", "school", "office", "home", "house", "apartment"]
    
    # Check if complaint contains non-railway context
    if any(keyword in complaint_text.lower() for keyword in non_railway_keywords):
        return {"MISCELLANEOUS": ["Others"]}

    # Proceed with AI classification if no non-railway keywords found
    prompt = (
...
# Sidebar setup
st.sidebar.image(r"C:\Users\visma\Downloads\Indian_Railway_Logo_2.png", width=250)
...
if choice == "File a Complaint":
    ...
    if input_method == "Record/Upload Audio":
        ...
        with col2:
            ...
            if uploaded_file:
                temp_input_path = tempfile.NamedTemporaryFile(delete=False, suffix=f".{uploaded_file.name.split('.')[-1]}").name
        ...
        if st.session_state["audio_path"]:
            ...
            if st.button("📩 Submit Audio Complaint", key="submit_audio"):
                ...
                complaint_data = {
                    ...
                    "input_type": "audio",
                    "complaint_text": edited_complaint,
                    "language": language
                }
                st.session_state["complaint_data"].append(complaint_data)
                st.session_state["audio_path"] = None
        ...
        for idx, complaint in enumerate(st.session_state["complaint_data"]):
            ...
            if st.button(f"Process Complaint {idx + 1}", key=f"process_{idx}"):
                ...
                complaint_data = {
                    ...
                    "station_phone": station_info["phone"]
                }
                save_to_db(complaint_data)

                for category, subcategories in category_to_subcategories.items():
                    send_complaint_email(category, subcategories, edited_text, complaint["phone_number"], complaint["pnr_number"], station_info)
